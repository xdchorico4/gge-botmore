...
async function getItemsJSON() {
  ...
  let str2 = undefined 
  try {
    str2 = fs.readFileSync("./ItemsVersion.properties").toString()
  }
  catch(e) {
    console.warn(e)
  }
  ...
  if(str != str2) {
      fs.writeFileSync("./ItemsVersion.properties", str)
      
      const response = await fetch(`https://empire-html5.goodgamestudios.com/default/items/items_v${str.match(new RegExp(/(?!.*=).*/))[0]}.json`);    
      
      fs.writeFileSync("./items.json", await response.text())
  }
}

const waitForItems = getItemsJSON()
let frame
